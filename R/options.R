...
#' @export
type_sum.tune_results <- function(x) {
...
#' @export
type_sum.resample_results <- function(x) {
   null_metrics <- purrr::map_lgl(x$.metrics, is.null)
   if (!all(null_metrics)) {
      res <- "rsmp[+]"
   } else {
...
#' @export
type_sum.workflow <- function(x) {
   if (x$trained) {
      fit <- workflows::pull_workflow_fit(x)
      if (inherits(fit$fit, "try-error")) {
         res <- "wflw[x]"
      } else {
         res <- "wflw[+]"
      }
